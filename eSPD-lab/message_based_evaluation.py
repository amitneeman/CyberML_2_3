...
datapack_path = "%s/annotated-datapack-%s-test-eval_mode-%s--window_size-%s.json" % (
    out_dir, args.dataset_indicator, args.segments_eval_mode, args.window_size)
...
datapack_path = "%s/annotated-datapack-%s-test-eval_mode-%s--window_size-%s.json" % (
    out_dir, args.dataset_indicator, args.full_eval_mode, args.window_size)
with open(datapack_path, "r") as file:
...
def evaluate_for_skepticism(skepticism):
    ...
    for chatName, chat in datapack_full["chats"].items():
        if not chat["className"] == "predator":
            continue
...
Path(eval_dir).mkdir(parents=True, exist_ok=True)
for name, scores in (
        {"f1": f1, "precision": precision, "recall": recall, "speed": speeds, "f_latency": f_latencies}).items():
    with open("%s/%s.txt" % (eval_dir, name), "w") as file:
        # files have 100 lines: of the values of the metrics at each message_num
        for val in scores: file.write("%s\n" % str(val))
