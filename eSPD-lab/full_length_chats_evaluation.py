...
from eval_util import getEvalArgs, getWarningLatency, isNonemptyMsg

###################
### This fils evaluates latency already predicted data
#################
...
datapackPath = "%s/annotated-datapack-%s-test-eval_mode-%s--window_size-%s.json" % (
    out_dir, args.dataset_indicator, args.eval_mode, args.window_size)
...
def evaluateForSkepticism(skepticism):
    ...
    for chatName in chatNames:
        ...
        if not latency:
            print("!!! the chat %s was not detected as positive" % chatName)
