...
# with open("ChatCoder2/raw_dataset/ArmySgt1961.xml", "r") as file:
...
def parseDate(dateString):
    ...
    try:
        return datetime.strptime(dateString, date_time_str)
    except:
        return None
...
for message in logsRoot:
    ...
    if isFromPredator:
        chat["numOfPredatorMessages"] += 1
    else:
        chat["numOfNonPredatorMessages"] += 1
...
print(
    "Number of messages with \n    noChatName %s, noLineNum %s, noUserID %s, noDateTime %s, badDateTime %s, noBody %s" % (
    noChatName, noLineNum, noUserID, noDateTime, badDateTime, noBody))
...
print("In total: %s chats with a total of %s messages, averaging %s messages per chat" % (
numberOfChats, numberOfMessages, round(numberOfMessages / numberOfChats)))
...
for label in labelsRoot:
    ...
    if groomingStage == "200":
        numberOfLabelsPredatorPI += 1  # "Personal Information (Predator)"
    elif groomingStage == "210":
        numberOfLabelsVictimPI += 1  # "Personal Information (Victim)"
    elif groomingStage == "600":
        numberOfLabelsPredatorG += 1  # "Grooming (Predator)"
    elif groomingStage == "610":
        numberOfLabelsVictimG += 1  # "Grooming (Victim)"
    elif groomingStage == "900":
        numberOfLabelsPredatorA += 1  # "Approach (Predator)"
    elif groomingStage == "910":
        numberOfLabelsVictimA += 1  # "Approach (Victim)"

    msg = chats[chatName]["content"][lineNum - 1]
    # keep in mind that a message can have multiple labels
    if groomingStage in ["200", "210"]:
        msg["labels"].add("PI")  # Personal Information
    elif groomingStage in ["600", "610"]:
        msg["labels"].add("G")  # Grooming
    elif groomingStage in ["900", "910"]:
        msg["labels"].add("A")  # Approach
...
print("%s messages (%s%%) were by predators" % (
numberOfPredatorMessages, round(100 * numberOfPredatorMessages / numberOfMessages)))
print(
    "%s messages (%s%%) are labeled. %s (%s%%) of the predator messages were labeled, %s (%s%%) of the victim messages were labeled" % (
